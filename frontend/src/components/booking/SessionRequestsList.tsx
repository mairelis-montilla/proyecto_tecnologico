...
const SessionRequestsList = () => {
    ...
    const confirmApprove = async () => {
        ...
        try {
            await bookingsService.approveBooking(selectedBooking._id)
            setShowApproveConfirm(false)
            fetchBookings() // Recargar lista
        } catch (err: any) {
            setActionError(err.response?.data?.message || 'Error al aprobar la sesión')
        } finally {
    ...
    const confirmReject = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setActionError(err.response?.data?.message || 'Error al rechazar la sesión')
        } finally {
            ...
            {selectedBooking && (
                <>
                    ...
                    <Modal
                        ...
                        onClose={() => !isApproving && setShowApproveConfirm(false)}
                        ...
                        <ModalBody>
                            <div className="text-center py-4">
                                ...
                                <p className="text-gray-600 mb-4">
                                    Aprobarás la sesión con <strong>{selectedBooking.studentId.userId.firstName}</strong> sobre "{selectedBooking.topic}"
                                </p>
                                {actionError && (
